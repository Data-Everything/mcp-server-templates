...
class FileServerConfig:
    ...
    def __init__(self):
        ...
        self.allowed_directories = [d.strip() for d in allowed_dirs_env.split(':') if d.strip()]
        
        # Other configuration options
        self.read_only = os.getenv('MCP_READ_ONLY', 'false').lower() == 'true'
        self.max_file_size = int(os.getenv('MCP_MAX_FILE_SIZE', str(10 * 1024 * 1024)))  # 10MB default
    ...
    def validate_path(self, path: str) -> Path:
        ...
        raise ValueError(f"Path '{path}' is not within allowed directories: {self.allowed_directories}")
...
class FileServer:
    ...
    def _register_tools(self) -> None:
        ...
        async def read_file(path: str) -> str:
            ...
            if validated_path.stat().st_size > self.config.max_file_size:
                raise ValueError(f"File too large: {validated_path.stat().st_size} > {self.config.max_file_size}")
        ...
        async def write_file(path: str, content: str) -> str:
            ...
            if len(content.encode('utf-8')) > self.config.max_file_size:
                raise ValueError(f"Content too large: {len(content)} > {self.config.max_file_size}")
        ...
        async def read_multiple_files(paths: List[str]) -> List[Dict[str, Any]]:
            ...
            for path in paths:
                try:
                    ...
                    if validated_path.stat().st_size > self.config.max_file_size:
                        raise ValueError(f"File too large: {validated_path.stat().st_size}")
        ...
        @self.mcp.tool()
        async def search_files(directory: str, pattern: str = "*", include_content: bool = False) -> List[Dict[str, Any]]:
            """
            ...
            for allowed_dir in self.config.allowed_directories:
                ...
                if dir_path.exists():
                    ...
                    directories.append({
                        ...
                        "writable": os.access(dir_path, os.W_OK) and not self.config.read_only,
                        "modified": stat.st_mtime
                    })
                else:
                    directories.append({
    ...
    def run(self) -> None:
        ...
        self.config.logger.info(f"Allowed directories: {self.config.allowed_directories}")
