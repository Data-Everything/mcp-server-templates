...
class TestMCPClientProbeDocker:
    ...
    @pytest.mark.asyncio
    @patch(
        "mcp_template.tools.mcp_client_probe.MCPClientProbe.discover_tools_from_command"
    )
    async def test_discover_tools_from_docker_mcp_with_env_vars(
        self, mock_discover_command
    ):
    ...
    async def test_discover_tools_from_docker_github_auto_stdio(
        self, mock_discover_command
    ):
    ...
    async def test_discover_tools_from_docker_with_custom_args(
        self, mock_discover_command
    ):
    ...
    def test_discover_tools_from_docker_sync_wrapper(self):
        ...
        with patch.object(self.probe, "discover_tools_from_docker_mcp") as mock_async:
            ...
            result = self.probe.discover_tools_from_docker_sync(
                "test/image:latest", args=["stdio"], env_vars={"TEST": "value"}
            )

            # Verify async method was called with correct arguments
            mock_async.assert_called_once_with(
                "test/image:latest", ["stdio"], {"TEST": "value"}
            )
    ...
    async def test_discover_tools_error_handling(self, mock_discover_command):
        ...
        result = await self.probe.discover_tools_from_docker_mcp(
            "failing/image:latest", env_vars={"KEY": "value"}
        )
    ...
    async def test_discover_tools_no_env_vars(self, mock_discover_command):
        ...
        result = await self.probe.discover_tools_from_docker_mcp(
            "simple/image:latest", args=["stdio"], env_vars=None
        )
    ...
    async def test_container_cleanup_logic(self):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            with patch.object(
                ...
                result = await self.probe.discover_tools_from_docker_mcp(
                    "test/image:latest", env_vars={"TEST": "value"}
                )
                ...
                cleanup_call = mock_subprocess.call_args_list[
                    -1
...
class TestMCPClientProbeProtocol:
    ...
    def test_normalize_mcp_tools_error_handling(self):
        ...
        assert (
            len(normalized) == 3
        )  # good_tool, unknown (missing name), another_good_tool
