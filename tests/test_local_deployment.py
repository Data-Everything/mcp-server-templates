...
def test_deployment():
    ...
    docker_cmd = [
        ...
        "--volume", "/tmp/mcp-test-data:/data",
        "--volume", "/tmp/mcp-test-workspace:/workspace",
        image_name
    ]
    ...
    status_cmd = ["docker", "ps", "--filter", f"name={container_name}", "--format", "table {{.Names}}\\t{{.Status}}\\t{{.Ports}}"]
    ...
    inspect_cmd = ["docker", "inspect", container_name, "--format", "{{.State.Running}}"]
    ...
    if running == "true":
        ...
        env_output = run_command(["docker", "exec", container_name, "env", "|", "grep", "MCP_"])
        if env_output:
            print(env_output)
        else:
            # Try without grep
