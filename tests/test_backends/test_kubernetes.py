...
class TestKubernetesDeploymentService:
    ...
    def test_init_creates_namespace(self, mock_core, mock_load_config):
        ...
        with patch('mcp_template.backends.kubernetes.client.AppsV1Api'), \
             patch('mcp_template.backends.kubernetes.client.AutoscalingV1Api'):
    ...
    def test_generate_deployment_name(self):
        ...
        with patch('mcp_template.backends.kubernetes.config.load_kube_config'), \
             patch('mcp_template.backends.kubernetes.client.AppsV1Api'), \
    ...
    def test_create_helm_values_http(self):
        ...
        with patch('mcp_template.backends.kubernetes.config.load_kube_config'), \
            ...
            values = service._create_helm_values("test", config, template_data)
    ...
    def test_create_helm_values_stdio(self):
        ...
        with patch('mcp_template.backends.kubernetes.config.load_kube_config'), \
            ...
            values = service._create_helm_values("stdio-test", config, template_data)
            
            assert values["mcp"]["type"] == "stdio"
            assert values["mcp"]["command"] == ["python", "server.py"]

    @patch('mcp_template.backends.kubernetes.KubernetesDeploymentService._wait_for_deployment_ready')
    @patch('mcp_template.backends.kubernetes.KubernetesDeploymentService._get_deployment_details')
    ...
    def test_list_deployments(self):
        ...
        with patch('mcp_template.backends.kubernetes.config.load_kube_config'), \
            ...
            with patch.object(service, '_get_deployment_details') as mock_get_details:
                mock_get_details.return_value = {"name": "test-deployment", "status": "running"}
