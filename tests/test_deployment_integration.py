...
class TestMCPDeploymentSystem:
    ...
    def _cleanup_test_containers(self):
        ...
        try:
            ...
            for deployment in deployments:
                try:
                    self.deployer.deployment_manager.delete_deployment(deployment["name"])
                except Exception:
    ...
    def test_template_discovery(self):
        ...
        for name, template in templates.items():
            ...
            for field in required_fields:
                assert field in template, f"Template {name} missing required field: {field}"
    ...
    def test_template_deployment(self):
        ...
        deployments = self.deployer.deployment_manager.list_deployments()
        template_deployments = [d for d in deployments if d["template"] == test_template]
        assert len(template_deployments) > 0, f"Deployed template {test_template} not found in list"
    ...
    def test_cleanup_functionality(self):
        ...
        assert len(remaining_deployments) <= initial_count, "Cleanup did not reduce deployment count"
