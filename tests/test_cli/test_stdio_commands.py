...
def test_run_stdio_tool_template_not_found(
    mock_docker_service, mock_console, enhanced_cli
):
...
@patch("mcp_template.cli.console")
@patch("mcp_template.cli.DockerDeploymentService")
def test_run_stdio_tool_non_stdio_template(
    mock_docker_service, mock_console, enhanced_cli
):
    ...
    assert result is False
    messages = [str(call) for call in mock_console.print.call_args_list]
...
@patch("mcp_template.cli.console")
@patch("mcp_template.cli.DockerDeploymentService")
def test_run_stdio_tool_invalid_json_args(
    mock_docker_service, mock_console, enhanced_cli
):
...
def test_deploy_with_transport_stdio_prevention(
    mock_deployer, mock_console, enhanced_cli
):
    ...
    with patch.object(enhanced_cli, "tool_discovery") as mock_tool_discovery:
        ...
        result = enhanced_cli.deploy_with_transport("github", transport="stdio")
