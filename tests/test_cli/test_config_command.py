...
class TestConfigCommand:
    ...
    def test_config_with_valid_template(self, cli, mock_args):
        ...
        with patch.object(cli.template_manager, 'get_template_config_schema', return_value=schema), \
             patch.object(cli.console, 'print') as mock_print:
            
            cli.handle_config_command(mock_args)
            
            # Verify schema was retrieved
            cli.template_manager.get_template_config_schema.assert_called_once_with("demo")
    ...
    def test_config_with_no_schema(self, cli, mock_args):
        ...
        with patch.object(cli.template_manager, 'get_template_config_schema', return_value=None), \
    ...
    def test_config_error_handling(self, cli, mock_args):
        ...
        with patch.object(cli, '_show_config_options', side_effect=Exception("Schema error")), \
    ...
    def test_config_complex_schema(self, cli, mock_args):
        ...
        with patch.object(cli.template_manager, 'get_template_config_schema', return_value=schema), \
            ...
            cli.template_manager.get_template_config_schema.assert_called_once_with("complex")
            
            # Should have printed multiple times for each property
            assert mock_print.call_count > 5  # At least one call per property plus headers
