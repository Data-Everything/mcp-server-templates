...
def test_common_modules():
    ...
    try:
        ...
        print(
            f"✅ TemplateManager.list_templates() returned {len(templates)} templates"
        )

        # Test config operations
        config_result = cm.merge_config_sources(
            template_config={"name": "test"}, config_values={"setting": "value"}
        )
        print(
            f"✅ ConfigManager.merge_config_sources() worked: {len(config_result)} keys"
        )
