...
class CoreMCPClient:
    ...
    def start_server(
        ...
        try:
            ...
            deployment_options = DeploymentOptions(
                name=name, transport=transport, port=port, timeout=timeout
            )
    ...
    def stream_server_logs(
        self, deployment_id: str, callback, lines: int = 100
    ) -> None:
        """
    ...
    def discover_tools(
        self, template_or_id: str, method: str = "auto", timeout: int = 30
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            return self.tool_manager.list_tools(
                template_or_id, discovery_method=method, timeout=timeout
            )
        except Exception as e:
    ...
    def get_template_configuration(self, template_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get template configuration for {template_name}: {e}"
            )
            return {}
    ...
    def update_template_configuration(
        self, template_name: str, config_updates: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def start_server_async(
        ...
        return await loop.run_in_executor(
            None, self.start_server, template_id, config, name, transport, port, timeout
        )
    ...
    async def list_tools_async(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self.list_tools, template_or_id, discovery_method, force_refresh
        )
    ...
    async def call_tool_async(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self.call_tool, template_or_deployment, tool_name, parameters, timeout
        )
