...
class TestGitLabToolValidation:
    ...
    def test_tool_categorization(self):
        ...
        expected_categories = {
            ...
            "Milestone Management": [
                ...
                "update_milestone", "close_milestone"
            ]
        }
    ...
    def test_expected_tool_count_validation(self):
        ...
        repo_tools = [
            "search_repositories", "create_repository", "get_project",
            "fork_project", "list_project_members", "create_or_update_file",
            "get_file_contents", "delete_file", "get_project_tree",
        ...
        additional_tools = [
            "list_project_variables", "create_project_variable",
            "get_project_variable", "update_project_variable",
            "delete_project_variable", "list_webhooks", "create_webhook"
        ]
        assert len(additional_tools) >= 5
        
        # Total expected: 65+ tools
        total_expected = (
            len(repo_tools) + len(issue_tools) + len(mr_tools) +
            len(pipeline_tools) + len(wiki_tools) + len(milestone_tools) +
    ...
    def test_readonly_mode_restrictions(self):
        ...
        write_tools = [
            "create_repository", "create_or_update_file", "delete_file",
            "create_issue", "update_issue", "close_issue",
            "create_merge_request", "merge_merge_request", "close_merge_request",
            "create_pipeline", "cancel_pipeline", "retry_pipeline",
        ...
        read_tools = [
            "search_repositories", "get_project", "get_file_contents",
    ...
    def test_environment_variable_consistency(self):
        ...
        for env_var, category in expected_env_vars.items():
            ...
            assert "_" in env_var or env_var.isalpha(), f"Environment variable {env_var} should use underscores"
            
            # Category should be valid
            valid_categories = ["authentication", "configuration", "feature_toggle", "transport", "networking"]
            assert category in valid_categories, f"Invalid category {category} for {env_var}"
    ...
    def test_tool_naming_conventions(self):
        ...
        sample_tools = [
            ...
            "list_merge_requests", "create_merge_request", "get_merge_request",
            "list_pipelines", "create_pipeline", "retry_pipeline",
            "list_wiki_pages", "create_wiki_page", "get_wiki_page",
        ...
        for tool in sample_tools:
            ...
            action_verbs = ["list", "get", "create", "update", "delete", "search", "retry", "cancel", "merge", "close", "reopen", "assign", "add", "remove"]
