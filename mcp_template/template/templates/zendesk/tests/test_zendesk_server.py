...
class TestZendeskMCPServer:
    ...
    def server(self, mock_config):
        ...
        with patch('server.ZendeskServerConfig') as mock_config_class:
            ...
            mock_config_instance.get_auth_headers.return_value = {"Authorization": "Basic test"}
    ...
    async def test_make_request_rate_limited(self, server):
        ...
        mock_rate_limited_response.text = AsyncMock(return_value='{"error": "Rate limited"}')
        
        # Second response: success
        mock_success_response = AsyncMock()
        mock_success_response.status = 200
        mock_success_response.text = AsyncMock(return_value=json.dumps(mock_response_data))
    ...
    async def test_get_ticket(self, server):
        ...
        server._make_request = AsyncMock(side_effect=[
            mock_ticket_response,
            mock_comments_response
        ])
    ...
    async def test_search_tickets(self, server):
        ...
        result = await server.search_tickets(
            query="test",
            status="open",
            limit=10
        )
    ...
    async def test_search_articles(self, server):
        ...
        result = await server.search_articles(
            query="test",
            locale="en-us"
        )
    ...
    async def test_get_ticket_metrics(self, server):
        ...
        mock_response = {
            "results": [
                {"id": 123, "status": "solved", "priority": "normal", "type": "question"},
                {"id": 124, "status": "open", "priority": "high", "type": "incident"},
                {"id": 125, "status": "closed", "priority": "normal", "type": "question"}
            ],
            "count": 3
        }
