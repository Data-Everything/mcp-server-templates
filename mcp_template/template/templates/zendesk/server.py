...
class ZendeskMCPServer:
    ...
    async def get_ticket(
        ...
        if include_comments:
            try:
                ...
                self.logger.warning(f"Failed to retrieve comments for ticket #{ticket_id}: {e}")
                response["comments"] = []
    ...
    async def search_articles(
        ...
        result = await self._make_request("GET", "help_center/articles/search.json", params=params)
    ...
    async def get_ticket_metrics(
        ...
        for ticket in tickets:
            ...
            metrics["type_breakdown"][ticket_type] = metrics["type_breakdown"].get(ticket_type, 0) + 1
        ...
        metrics["resolution_stats"] = {
            ...
            "resolution_rate": len(solved_tickets) / len(tickets) * 100 if tickets else 0,
            "open_count": len([t for t in tickets if t.get("status") in ["new", "open", "pending", "hold"]])
        }
    ...
    async def list_organizations(
        ...
        if query:
            ...
            result = await self._make_request("GET", "search.json", params=params)
            organizations = [r for r in result.get("results", []) if r.get("result_type") == "organization"]
        else:
