...
class ConfigManager:
    ...
    def merge_config_sources(self,
                           ...
                           override_values: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        """
    ...
    def validate_config(self, config: Dict[str, Any], schema: Dict[str, Any]) -> ValidationResult:
        """
    ...
    def load_configuration_for_template(self, template_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if template_config_file.exists():
                configurations["template"] = self._load_config_file(str(template_config_file))
            ...
            if config_dir.exists():
                for config_file in config_dir.glob("*.json"):
                    ...
                    configurations[config_name] = self._load_config_file(str(config_file))
            ...
            logger.error(f"Failed to load configurations for template {template_name}: {e}")
            return {}
        ...
        try:
            ...
            for config_name, config_data in configurations.items():
                ...
                if not isinstance(config_data, dict):
                    all_errors.append(f"Configuration '{config_name}' is not a valid object")
                    continue
                ...
                if config_name == "template":
                    ...
                    for field in required_fields:
                        if field not in config_data:
                            all_errors.append(f"Template configuration missing required field: {field}")
    ...
    def _process_env_vars(self, env_vars: Dict[str, str], context_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _apply_overrides(self, config: Dict[str, Any], overrides: Dict[str, str]) -> Dict[str, Any]:
        """Apply overrides with double-underscore notation."""
        result = config.copy()
    ...
    def _validate_field(self, field_name: str, value: Any, field_schema: Dict[str, Any]) -> List[str]:
        ...
        if isinstance(value, str):
            ...
            if min_length and len(value) < min_length:
                errors.append(f"Field '{field_name}' is too short (minimum {min_length} characters)")
                
            max_length = field_schema.get("maxLength")
            if max_length and len(value) > max_length:
                errors.append(f"Field '{field_name}' is too long (maximum {max_length} characters)")
