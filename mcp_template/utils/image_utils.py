...
def get_image_registry(image_name: str) -> Optional[str]:
    """Extract the registry from an image name.

    Args:
        image_name: The image name to extract registry from

    Returns:
        Registry name or None if no registry specified

    Examples:
        get_image_registry("docker.io/nginx") -> "docker.io"
        get_image_registry("nginx") -> None
        get_image_registry("gcr.io/project/image") -> "gcr.io"
    """
...
def is_local_image(image_name: str) -> bool:
    """Check if an image name refers to a local image.

    Args:
        image_name: The image name to check

    Returns:
        True if the image appears to be local (no registry or localhost)
    """
    if not image_name:
        return False

    # Check for localhost registry
    if image_name.startswith("localhost"):
        return True

    # Check if it has no registry (simple name like "nginx")
    if "/" not in image_name:
        return True

    # Check if the first part doesn't look like a registry
    first_part = image_name.split("/")[0]
