...
class RichHelpFormatter:
    ...
    def format_help(self, parser: argparse.ArgumentParser) -> None:
        ...
        subtitle = Text("Deploy MCP server templates with zero configuration", style="italic")
    ...
    def _print_commands_tree(self, parser: argparse.ArgumentParser) -> None:
        ...
        categories = {
            ...
            "⚙️  Configuration & Tools": ["config", "tools", "discover-tools", "examples", "run-tool"],
            ...
            for command_name in command_names:
                if command_name in subparsers.choices and command_name in commands_info:
                    ...
                    if "Deprecated" in help_text:
                        command_branch = category_branch.add(f"[dim]{command_name}[/dim] - [yellow]{help_text}[/yellow]")
                    else:
                        command_branch = category_branch.add(f"[green]{command_name}[/green] - {help_text}")
