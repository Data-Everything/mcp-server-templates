...
class ClientTestRunner:
    ...
    def test_client_initialization(self):
        ...
        try:
            ...
            self.log_test_result("Invalid backend initialization", True, "Gracefully handled")
    ...
    def test_template_methods(self):
        ...
        try:
            ...
            self.log_test_result("list_templates()", True, f"Found {len(templates)} templates")
            ...
            self.log_test_result("get_template_info(nonexistent)", True, "Correctly returned None")
            
            # Test validate_template for existing template
            if "demo" in templates:
                is_valid = client.validate_template("demo")
                assert isinstance(is_valid, bool)
                self.log_test_result("validate_template('demo')", True, f"Valid: {is_valid}")
            
            # Test validate_template for non-existent template
            is_invalid = client.validate_template("nonexistent_template_12345")
            assert is_invalid is False
            self.log_test_result("validate_template(nonexistent)", True, "Correctly returned False")
            
            # Test search_templates
            search_results = client.search_templates("demo")
            assert isinstance(search_results, dict)
            self.log_test_result("search_templates('demo')", True, f"Found {len(search_results)} matches")
            ...
            self.log_test_result("search_templates(no matches)", True, f"Found {len(no_matches)} matches")
    ...
    def test_server_management(self):
        ...
        try:
            ...
            self.log_test_result("list_servers()", True, f"Found {len(servers)} servers")
            
            # Test list_servers_by_template
            demo_servers = client.list_servers_by_template("demo")
            assert isinstance(demo_servers, list)
            self.log_test_result("list_servers_by_template('demo')", True, f"Found {len(demo_servers)} demo servers")
            ...
            if server_result:
                ...
                self.log_test_result("start_server('demo')", True, f"Started: {deployment_id}")
                ...
                if server_info:
                    self.log_test_result("get_server_info()", True, f"Status: {server_info.get('status')}")
                else:
                    self.log_test_result("get_server_info()", False, "No server info returned")
                
                # Test get_server_logs
                logs = client.get_server_logs(deployment_id)
                if logs:
                    self.log_test_result("get_server_logs()", True, f"Got {len(logs.split())} log lines")
                else:
                    self.log_test_result("get_server_logs()", True, "No logs yet (normal for new server)")
                ...
                if stop_result.get("success"):
                    ...
                    self.log_test_result("stop_server()", False, stop_result.get("error", "Unknown error"))
            else:
                self.log_test_result("start_server('demo')", False, "Failed to start server")
            ...
            self.log_test_result("start_server(nonexistent)", True, "Correctly returned None")
    ...
    def test_tool_operations(self):
        ...
        try:
            ...
            self.log_test_result("list_tools('demo')", True, f"Found {len(tools)} tools")
            ...
            self.log_test_result("list_tools(force_refresh=True)", True, f"Found {len(refreshed_tools)} tools")
            ...
            self.log_test_result("list_tools(nonexistent)", True, f"Found {len(no_tools)} tools")
            ...
            if server_result:
                ...
                if tools:
                    ...
                    tool_result = client.call_tool(deployment_id, tool_name, {"message": "test"})
                    if tool_result and tool_result.get("success"):
                        self.log_test_result("call_tool()", True, f"Tool {tool_name} executed successfully")
                    else:
                        self.log_test_result("call_tool()", False, tool_result.get("error", "Unknown error"))
                else:
                    self.log_test_result("call_tool()", True, "No tools available to test")
                
                # Test call_tool with invalid tool name
                invalid_tool = client.call_tool(deployment_id, "nonexistent_tool_xyz", {})
                ...
                self.log_test_result("call_tool()", False, "Could not start server for testing")
            
            # Test call_tool with invalid deployment
            invalid_call = client.call_tool("invalid_deployment_xyz", "echo", {"message": "test"})
            assert not invalid_call.get("success", True)
            self.log_test_result("call_tool(invalid deployment)", True, "Correctly failed")
    ...
    async def test_connection_operations(self):
        ...
        try:
            ...
            if connection_id:
                self.log_test_result("connect_stdio()", True, f"Connection: {connection_id}")
                
                # Test list_tools_from_connection
                conn_tools = await client.list_tools_from_connection(connection_id)
                if conn_tools is not None:
                    self.log_test_result("list_tools_from_connection()", True, f"Found {len(conn_tools)} tools")
                else:
                    self.log_test_result("list_tools_from_connection()", True, "No tools (expected for echo command)")
                
                # Test call_tool_from_connection
                call_result = await client.call_tool_from_connection(connection_id, "test_tool", {})
                # This will likely fail since echo doesn't support MCP protocol
                self.log_test_result("call_tool_from_connection()", True, "Attempted call (expected to fail with echo)")
                ...
                self.log_test_result("connect_stdio()", False, "Failed to create connection")
            ...
            self.log_test_result("connect_stdio(invalid)", True, "Correctly returned None")
            ...
            self.log_test_result("list_tools_from_connection(invalid)", True, "Correctly returned None")
            
            no_call = await client.call_tool_from_connection("nonexistent_conn_xyz", "tool", {})
            assert no_call is None
            self.log_test_result("call_tool_from_connection(invalid)", True, "Correctly returned None")
            
            no_disconnect = await client.disconnect("nonexistent_conn_xyz")
            assert no_disconnect is False
            self.log_test_result("disconnect(invalid)", True, "Correctly returned False")
            ...
            async with MCPClient() as ctx_client:
                ...
                if test_conn:
                    ...
                    pass
            self.log_test_result("context manager", True, "Successfully used as context manager")
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            try:
                ...
                self.log_test_result("get_template_info(None)", True, "Handled None input")
            except Exception:
                self.log_test_result("get_template_info(None)", True, "Raised exception (acceptable)")
            ...
            self.log_test_result("get_template_info(long_string)", True, "Handled long input")
            
            # Test with special characters
            special_chars = "template!@#$%^&*()_+-=[]{}|;':\",./<>?"
            special_result = client.get_template_info(special_chars)
            assert special_result is None
            self.log_test_result("get_template_info(special_chars)", True, "Handled special characters")
    ...
    def test_performance(self):
        ...
        try:
            ...
            self.log_test_result("repeated list_templates", True, f"10 calls in {duration:.2f}s")
            ...
            try:
                ...
                self.log_test_result("memory usage", True, f"Memory change: {memory_diff:.1f}MB")
            except ImportError:
                self.log_test_result("memory usage", True, "psutil not available (skipped)")
